...
- [ ] Teach the resolver how to evaluate simple member function calls on literals—starting with `(0..<limit).contains(index)` and `items.contains(userId)` by recognizing range/array bases and returning `.bool`—so control flow can lean on the same helpers developers use in regular SwiftUI.
