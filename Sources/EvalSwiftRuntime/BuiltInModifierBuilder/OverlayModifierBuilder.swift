...
public struct OverlayModifierBuilder: RuntimeModifierBuilder {
    public let modifierName = "overlay"
    ...
    public func applyModifier(
        ...
        if let function = contentFunction {
            guard let overlay = function.value.asSwiftUIView else {
                throw RuntimeError.invalidViewArgument("overlay modifier expects a view or closure providing one.")
            }
            return AnyView(view.overlay(alignment: alignment) { overlay })
        }
